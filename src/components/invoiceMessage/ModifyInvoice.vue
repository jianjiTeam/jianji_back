...
<!--修改发票对话框组件-->
<template>
  <div class="ModifyInvoice">
    <!--发票修改项-->
    <el-form label-position="left" label-width="60px">

      <el-form-item label="发票号">
        <el-col :span="9">
          <el-input disabled v-model="form.invoiceNumber" placeholder=""></el-input>
        </el-col>
      ...
      <el-form-item>
        <el-button @click="$parent.$parent.dialog.showModify = false" class="float-right">取消</el-button>
        <el-button :loading="myLoading" @click="clickSaveBtn" style="margin-right: 20px" class="float-right" type="primary">保存</el-button>
      </el-form-item>
    </el-form>
    <!--中间分割线-->
    <div class="line"></div>
...
<script>
  export default {
    ...
    methods: {
      clickSaveBtn () {
        this.myLoading = true
        this.$http.postInvoices(this.form).then(({data}) => {
          this.myLoading = false
          if (!data) {
            // 数据回流
            this.$emit('input', this.form)
            // 找到对话框的父组件，来设置隐藏显示
            this.$parent.$parent.dialog.showModify = false
            // 成功后的提示
            this.$message({
              message: '开票成功'
            })
